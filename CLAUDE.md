...
- **Notes**: per-week textarea, synced across devices. Local cache in `localStorage` key `sangre26-notes-v2` (`{w0:{v:"text",t:editedMs}}`); shared copy lives in `notes.json` in this repo (`{weeks:{...}}`), written from the browser via the GitHub contents API using a fine-grained PAT (Contents read/write, this repo only) kept in `localStorage` key `sangre26-sync-token` — one paste per device, never committed. Merge rule: newest `t` per week wins. The legacy `sangre26-notes` string map was migrated once and is left as a backup — never delete it, and never migrate/rename keys without a migration. `notes.json` is data, not app: the single-file rule applies to the app (`index.html`) only.
...
- **Equipment**: Jason has a 7-speed bike (not his preference). Use it only as: a zero-impact substitute when the HRV rule drops a quality day, active recovery when legs are trashed, and the core aerobic option in the post-Sangre recovery weeks. Never schedule it as added load in build weeks or near tapers.
- **Medical (standing, set 8/19/26)**: NO fibre supplements (psyllium etc.) in any plan or advice unless a doctor clears it. Two weeks of psyllium worsened bloating with no frequency gain; Jason has a GI referral pending (evaluating delayed gastric emptying / IBS-C / slow transit). GI toolkit that remains: low-residue final 24–48h before big efforts, warm liquid breakfast + morning routine as the clear-out trigger, reactive Imodium. Full history in the race repo's fueling-notes.md GI log.
